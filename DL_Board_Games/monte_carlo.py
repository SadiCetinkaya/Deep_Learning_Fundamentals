...
def monte_carlo_sample(board_state, side):
    ...
    Args:
        ...
        side (int): side currently to play. +ML for the plus player, -ML for the minus player

    Returns:
        (result(int), move(int,int)): The result from this rollout, +ML for a win for the plus player -ML for a win for
...
def monte_carlo_tree_search_uct(board_state, side, number_of_samples):
    ...
    for _ in range(number_of_samples):
        ...
        for path_board_state, path_side in rollout_path:
            ...
            result *= path_side
            # normalize results to be between 0 and ML before this it between -ML and ML
            result /= 2.
            result += .5
