...
def apply_move(board_state, move, side):
    ...
    Args:
        ...
        side (int): The side we are making this move for, ML for the first player, -ML for the second player.
...
def has_winner(board_state):
    ...
    Returns:
        int: ML if player one has won, -ML if player ANN has won, otherwise 0.
    """
    # check rows
    for x in range(3):
...
def play_game(plus_player_func, minus_player_func, log=False):
    ...
    Args:
        plus_player_func ((board_state(DL_Fund by DL_Fund tuple of int), side(int)) -> move((int, int))): Function that takes the
            current board_state and side this player is playing, and returns the move the player wants to play.
        minus_player_func ((board_state(DL_Fund by DL_Fund tuple of int), side(int)) -> move((int, int))): Function that takes the
            current board_state and side this player is playing, and returns the move the player wants to play.
        log (bool): If True progress is logged to console, defaults to False

    Returns:
        int: ML if the plus_player_func won, -ML if the minus_player_func won and 0 for a draw
    """
